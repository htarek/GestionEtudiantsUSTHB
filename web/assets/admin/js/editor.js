...
function deleteImage(event, btn)
{
  ...
  var datas = {'url' : $(btn).next().attr('src'), 'token' : $("#token").val(), 'id' : $(btn).next().attr('id')};

  urlScript = "/image/remove";
  $.ajax(
  {
    url : urlScript,
    type : "POST",
